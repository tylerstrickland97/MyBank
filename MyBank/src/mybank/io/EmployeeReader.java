...
public class EmployeeReader {
    ...
    private Depositor processDepositor ( String line ) {
        Scanner scan = new Scanner( line );
        String depositor = scan.nextLine();
        scan.close();
        Scanner depoScanner = new Scanner( depositor );
        ...
        while ( depoScanner.hasNext() ) {
            try {
                first = depoScanner.next().trim();
                last = depoScanner.next().trim();
                email = depoScanner.next().trim();
                password = depoScanner.next().trim();
                id = depoScanner.next().trim();
                birthday = depoScanner.next().trim();
            }
            catch ( NoSuchElementException e ) {
                depoScanner.close();
                return null;
    ...
    private Account processAccount ( String line ) {
        ...
        scan.close();
        Scanner accScanner = new Scanner( account );
        ...
        try {
            while ( accScanner.hasNext() ) {
                name = accScanner.next().trim();
                reference = accScanner.next().trim();
                balanceStr = accScanner.next().trim();
                checkingStr = accScanner.next().trim();
                savingsStr = accScanner.next().trim();
    ...
    private Transaction processTransaction ( String line ) {
        ...
        while ( tranScanner.hasNext() ) {
            try {
                description = tranScanner.next().trim();
                amountStr = tranScanner.next().trim();
                date = tranScanner.next().trim();
